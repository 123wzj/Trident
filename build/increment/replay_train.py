"""Replay-based incremental training for dual-branch APT attribution."""
...
OUTPUT_DIR = Path("./incremental_results")
...
class Config:
    ...
    ioc_patience: int = 50
    ioc_lr: float = 5e-4
    ttp_max_epochs: int = 150
    ttp_patience: int = 40
    ttp_lr: float = 3e-4

    finetune_epochs: int = 100
    finetune_patience: int = 25
    ioc_finetune_lr: float = 8e-4
    ttp_finetune_lr: float = 2e-4
    replay_ratio: float = 0.2
    old_loss_weight: float = 1.0

    replay_warmup_ratio: float = 0.05
    replay_mid_ratio: float = 0.20
    replay_late_ratio: float = 0.35
    ...
    def show(self):
        W = 58
        print("\n" + "=" * W)
        print("  Replay incremental training config".center(W))
        print("=" * W)
        for k, v in [
            ("device", self.device),
            ("seed", self.seed),
            ...
            (
                "replay warmup/mid/late",
                f"{self.replay_warmup_ratio}/{self.replay_mid_ratio}/{self.replay_late_ratio}",
            ),
...
class IOCClassifier(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict, num_nodes_dict):
        ref_dev = next(iter(x_dict.values())).device if x_dict else torch.device("cpu")
        h_dict = {}
        for nt in self.node_types:
            x = x_dict.get(nt)
            n = num_nodes_dict.get(nt, 1)
            h_dict[nt] = (
                self.input_projs[nt](x)
                if x is not None and nt in self.input_projs
                else torch.zeros(max(n, 1), self.hidden_dim, device=ref_dev)
            )
        ...
        for conv, norm in zip(self.convs, self.norms):
            ...
            deg.scatter_add_(
                0, dst.unsqueeze(1).expand(-1, h.shape[1]), torch.ones_like(em)
            )
...
class TTPTransformer(nn.Module):
    def __init__(
        ...
        if pretrained_embeddings is not None:
            ...
            if vocab_sz < SAFE_VOCAB_SIZE:
                ext = torch.cat(
                    [
                        ...
                        torch.zeros(SAFE_VOCAB_SIZE - vocab_sz, sem_dim),
                    ],
                    dim=0,
                )
            else:
            ...
            self.semantic_proj = (
                nn.Linear(sem_dim, d_model) if sem_dim != d_model else nn.Identity()
            )
        else:
            self.tech_embedding = nn.Embedding(SAFE_VOCAB_SIZE, d_model, padding_idx=0)
    ...
    def forward(self, tech_seqs, phase_seqs, global_feats, mask):
        ...
        if phase_seqs is not None:
            ...
            x = self.fusion_proj(torch.cat([x, self.phase_embedding(ps)], dim=-1))
...
def ioc_loader(ioc_data, node_idx, cfg, shuffle):
    return NeighborLoader(
        ...
        input_nodes=("EVENT", torch.tensor(node_idx, dtype=torch.long)),
        shuffle=shuffle,
    )


def train_ioc(
    model, ioc_data, g_tr, g_va, cw, crit, max_ep, lr, patience, cfg, device, tag=""
):
    n_cls = len(cw)
...
def _val_ioc(model, va_ld, device):
    ...
    with torch.no_grad():
        for b in va_ld:
            ...
            bs = b["EVENT"].batch_size
            xi, ei, ea, nn_ = ioc_inputs(b)
            preds.append(model.classifier(model(xi, ei, ea, nn_)[:bs]).argmax(1).cpu())
            lbls.append(b["EVENT"].y[:bs].cpu())
    return balanced_accuracy_score(torch.cat(lbls).numpy(), torch.cat(preds).numpy())
...
def train_ttp(
    ...
    tr_pos = [
        i
        for i in (list(tr_pos) if hasattr(tr_pos, "tolist") else list(tr_pos))
        if 0 <= int(labels[i]) < nc
    ]
    va_pos = list(va_pos) if hasattr(va_pos, "tolist") else list(va_pos)

    tr_ld = DataLoader(
        list(zip(tr_pos, labels[tr_pos].tolist())),
        batch_size=32,
        shuffle=True,
    ...
    for ep in range(max_ep):
        ...
        with torch.no_grad():
            bacc = balanced_accuracy_score(
                vlbl.cpu().numpy(), model(vp, vph, vg, vm).argmax(1).cpu().numpy()
            )
        sch.step(bacc)
        if bacc > best_bacc:
            best_bacc = bacc
        ...
        if (ep + 1) % 20 == 0:
            print(
                f"      [{tag}] Ep{ep+1:03d} B-Acc={bacc:.4f} "
...
def get_replay_ratio(ep: int, total_ep: int, cfg) -> float:
    """Get replay ratio."""
    stage = ep / total_ep
    if stage < 1 / 3:
        return cfg.replay_warmup_ratio
    elif stage < 2 / 3:
        return cfg.replay_mid_ratio
    else:
        return cfg.replay_late_ratio
...
def replay_ioc(
    ...
    for arr, dat, which in [(g_tr_new, new_ioc, "new"), (g_tr_old, old_ioc, "old")]:
        bad = (dat["EVENT"].y[arr].cpu().numpy() < 0) | (
            dat["EVENT"].y[arr].cpu().numpy() >= nc
        )
        if bad.any():
            if which == "new":
                g_tr_new = g_tr_new[~bad]
            else:
                g_tr_old = g_tr_old[~bad]

    va_ld = ioc_loader(new_ioc, g_va_new, cfg, shuffle=False)
    opt = torch.optim.AdamW(
        model.parameters(), lr=cfg.ioc_finetune_lr, weight_decay=cfg.weight_decay
    )
    ...
    for ep in range(cfg.finetune_epochs):
        ratio = get_replay_ratio(ep, cfg.finetune_epochs, cfg)
        if ratio != cur_ratio:
            cur_ratio = ratio
            old_bs = max(4, int(cfg.batch_size * ratio))
            new_bs = max(4, cfg.batch_size - old_bs)
            new_ld = NeighborLoader(
                new_ioc,
                num_neighbors=cfg.num_neighbors,
                batch_size=new_bs,
                input_nodes=("EVENT", torch.tensor(g_tr_new, dtype=torch.long)),
                shuffle=True,
            )
            old_ld = NeighborLoader(
                old_ioc,
                num_neighbors=cfg.num_neighbors,
                batch_size=old_bs,
                input_nodes=("EVENT", torch.tensor(g_tr_old, dtype=torch.long)),
                shuffle=True,
            )
            if ep > 0:
                print(
                    f"      [{tag}] Ep{ep+1:03d} ratio {ratio:.2f} "
                    f"(new_bs={new_bs}, old_bs={old_bs})"
                )
        ...
        for nb in new_ld:
            nb = nb.to(device)
            bs = nb["EVENT"].batch_size
            lbl = nb["EVENT"].y[:bs]
            if (lbl < 0).any() or (lbl >= nc).any():
                continue
        ...
        if bacc > best_bacc:
            ...
            if pat >= cfg.finetune_patience:
                print(
                    f"    [{tag}] Early stop ep{ep+1} "
                    f"B-Acc={best_bacc:.4f} ratio={cur_ratio:.2f}"
                )
                break
        if (ep + 1) % 20 == 0:
            print(
                f"      [{tag}] Ep{ep+1:03d} B-Acc={bacc:.4f} " f"ratio={cur_ratio:.2f}"
            )
...
def replay_ttp(
    ...
    model = deepcopy(base_model).to(device)
    nc = model.num_classes
    os_ = ttp_seqs(old_ttp)
    oph = old_ttp.get("phase_sequences")
    ogf = old_ttp.get("global_features")
    ns_ = ttp_seqs(new_ttp)
    nph = new_ttp.get("phase_sequences")
    ...
    opt = torch.optim.AdamW(
        model.parameters(), lr=cfg.ttp_finetune_lr, weight_decay=cfg.weight_decay
    )
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(
    ...
    for ep in range(cfg.finetune_epochs):
        ...
        if ratio != cur_ratio:
            cur_ratio = ratio
            nbs = 24
            obs = max(4, int(nbs * ratio))
            new_ld = DataLoader(
                list(zip(tr_new, lbl_new[tr_new].tolist())),
                batch_size=nbs,
                shuffle=True,
                collate_fn=lambda b: list(zip(*b)),
            )
            old_ld = DataLoader(
                list(zip(tr_old, lbl_old[tr_old].tolist())),
                batch_size=obs,
                shuffle=True,
                collate_fn=lambda b: list(zip(*b)),
            )
        ...
        for bidx, blbl in new_ld:
            ...
            if vo.any():
                if not vo.all():
                    ...
                    if og is not None:
                        og = og[vo]
                loss = loss + cfg.old_loss_weight * crit(model(op, oph_, og, om), olab)
        ...
        if (ep + 1) % 20 == 0:
            print(
                f"      [{tag}] Ep{ep+1:03d} B-Acc={bacc:.4f} "
                f'LR={opt.param_groups[0]["lr"]:.6f} ratio={cur_ratio:.2f}'
            )
...
def full_finetune(
    ...
    opt = torch.optim.AdamW(
        m_ioc.parameters(), lr=cfg.ioc_lr, weight_decay=cfg.weight_decay
    )
    sch = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opt, mode="max", factor=0.5, patience=15
    )
    ...
    for ep in range(cfg.ioc_max_epochs):
        ...
        _run(ld_o)
        _run(ld_n)
        bacc = _val_ioc(m_ioc, ld_v, device)
        sch.step(bacc)
        if bacc > best:
            best = bacc
            sd = deepcopy(m_ioc.state_dict())
            pat = 0
        else:
            pat += 1
            if pat >= cfg.ioc_patience:
                print(f"    [Full-IOC] Early stop ep{ep+1} B-Acc={best:.4f}")
                break
        if (ep + 1) % 30 == 0:
    ...
    og_t = (
        (ogf if isinstance(ogf, torch.Tensor) else torch.tensor(ogf))
        if ogf is not None
        else None
    )
    ng_t = (
        (ngf if isinstance(ngf, torch.Tensor) else torch.tensor(ngf))
        if ngf is not None
        else None
    )
    c_g = torch.cat([og_t, ng_t], 0) if og_t is not None and ng_t is not None else None
    ...
    sk = "causal_sequences" if "causal_sequences" in new_ttp else "technique_sequences"
    c_ttp = {
        sk: c_seq,
        "phase_sequences": c_ph,
        "global_features": c_g,
        "num_techniques": max(
            old_ttp.get("num_techniques", 370), new_ttp.get("num_techniques", 370)
        ),
...
def eval_ioc(model, ioc_data, g_te, cfg, device):
    ...
    lb = torch.cat(lbl_all).numpy()
    probs = F.softmax(lg, dim=1).numpy()
    pr = lg.argmax(1).numpy()
    return accuracy_score(lb, pr), balanced_accuracy_score(lb, pr), pr, lb, probs
...
def eval_ttp(model, ttp_data, te_pos, labels, cfg, device):
    ...
    te_y = labels[te_pos]
    with torch.no_grad():
        logits = model(tp, tph, tg, tm)
        probs = F.softmax(logits, dim=1).cpu().numpy()
        pr = logits.argmax(1).cpu().numpy()
    return accuracy_score(te_y, pr), balanced_accuracy_score(te_y, pr), pr, te_y, probs


def branch_confidence(probs):
    """Estimate confidence with normalized negative entropy."""
...
def entropy_aware_fusion(graph_probs, seq_probs):
    """Fuse posterior distributions using entropy-derived dynamic weights."""
    graph_conf = branch_confidence(graph_probs)
    seq_conf = branch_confidence(seq_probs)
    ...
    fused_probs = (
        graph_weight[:, None] * graph_probs + (1.0 - graph_weight[:, None]) * seq_probs
    )
    fused_probs = fused_probs / np.clip(fused_probs.sum(axis=1, keepdims=True), 1e-12, None)
...
def get_metrics(im, tm, ioc_d, ttp_d, g_te, pos_te, labels, cfg, device):
    ...
    ia, ib, ip, il, i_probs = eval_ioc(im, ioc_d, g_te, cfg, device)
    ta, tb, tp_, tl, t_probs = eval_ttp(tm, ttp_d, pos_te, labels, cfg, device)
    n = min(len(ip), len(tp_))
    if not np.array_equal(il[:n], tl[:n]):
        raise ValueError("IOC/TTP labels are not aligned for decision fusion.")
    fusion_probs, graph_weight, graph_conf, seq_conf = entropy_aware_fusion(
        i_probs[:n], t_probs[:n]
    )
    fusion_pred = fusion_probs.argmax(axis=1)
    oracle_mask = (ip[:n] == il[:n]) | (tp_[:n] == il[:n])
    return {
        "ioc_acc": float(ia),
        "ioc_bacc": float(ib),
        "ttp_acc": float(ta),
        "ttp_bacc": float(tb),
        "or_acc": float(accuracy_score(il[:n], fusion_pred)),
        "or_bacc": float(balanced_accuracy_score(il[:n], fusion_pred)),
        "or_f1_macro": float(f1_score(il[:n], fusion_pred, average="macro")),
...
def cl_metrics(summary, base_k, inc_k, full_k):
    ...
    def m(k, sp):
        return summary[k][sp]["or_acc_mean"]

    fm = m(base_k, "old") - m(inc_k, "old")
    bt = -fm
    ft = m(base_k, "new")
    intr = m(full_k, "new") - m(inc_k, "new")
    psr = m(inc_k, "new") / m(inc_k, "old") if m(inc_k, "old") > 0 else 0.0
    return {
        "FM": round(fm, 4),
        "BT": round(bt, 4),
        "FT": round(ft, 4),
        "Intransigence": round(intr, 4),
        "PSR": round(psr, 4),
    }
...
def filter_to_classes(data, target_classes):
    ...
    if not found:
        return data
    tset = set(tl)
    o2n = {old: new for new, old in enumerate(tl)}
    ...
    print(f"    kept samples: {kept}, classes: {len(data._apt_classes)}")
    return data
...
def filter_top_k(data, k=15):
    ...
    cur = data._apt_classes if hasattr(data, "_apt_classes") else []
    tgt = (
        [cur[l] for l in top_k if l < len(cur)]
        if len(cur) > 0
...
def align_valid_field(field, valid_idx, total_events, field_name):
    """Align a tensor/list field to valid EVENT space."""
    if field is None:
        return None
    field_len = len(field)
    ...
    raise ValueError(
        f"{field_name} length mismatch: got {field_len}, expected {valid_len} or {total_events}."
    )


def filter_ttp(ttp_data, ioc_data):
    vi = torch.where(ioc_data["EVENT"].y != -1)[0].numpy()
    sk = "causal_sequences" if "causal_sequences" in ttp_data else "technique_sequences"
    nc = ioc_data._apt_classes if hasattr(ioc_data, "_apt_classes") else []
    gf = ttp_data.get("global_features")
    total_events = len(ioc_data["EVENT"].y)
    sequences = align_valid_field(ttp_data[sk], vi, total_events, sk)
    phases = align_valid_field(
        ttp_data.get("phase_sequences"), vi, total_events, "phase_sequences"
    )
    global_features = align_valid_field(gf, vi, total_events, "global_features")
    d = {
        sk: sequences,
    ...
    print(f"    TTP filtered samples: {len(vi)}")
    return d


def parse_args():
    p = argparse.ArgumentParser(description="APT dual-branch replay experiment")
    p.add_argument("--old-loss-weight", type=float, default=1.0)
    p.add_argument("--replay-warmup-ratio", type=float, default=0.05)
    p.add_argument("--replay-mid-ratio", type=float, default=0.20)
    p.add_argument("--replay-late-ratio", type=float, default=0.35)
    p.add_argument(
        "--results-dir", type=str, default="./incremental_results/replay_final"
    )
...
def main():
    args = parse_args()
    print("\n" + "=" * 65)
    print("  APT Experience Replay".center(65))
    print("=" * 65)
    ...
    print(
        f'    old IOC EVENT nodes: {old_ioc["EVENT"].num_nodes:,}   '
        f'updated IOC EVENT nodes: {new_ioc["EVENT"].num_nodes:,}'
    )

    print("\n  [2/5] Filtering classes and aligning branches...")
    TOP_K = 15
    old_ioc = filter_top_k(old_ioc, TOP_K)
    sel_cls = old_ioc._apt_classes.tolist() if hasattr(old_ioc, "_apt_classes") else []
    new_ioc = filter_to_classes(new_ioc, sel_cls)

    old_vi = torch.where(old_ioc["EVENT"].y != -1)[0]
    new_vi = torch.where(new_ioc["EVENT"].y != -1)[0]
    inc_idx = (
        new_vi[new_vi > old_vi.max().item()]
        if len(old_vi) > 0 and len(new_vi) > 0
        else new_vi
    )
    print(f"    selected classes: {len(sel_cls)}")
    print(
        f"    old valid samples: {len(old_vi):,}  incremental samples: {len(inc_idx):,}"
    )
    ...
    sk = (
        "causal_sequences" if "causal_sequences" in old_ttp_f else "technique_sequences"
    )
    old_n = int((old_ioc["EVENT"].y != -1).sum())
    new_n = int((new_ioc["EVENT"].y != -1).sum())
    if old_n != len(old_ttp_f[sk]) or new_n != len(new_ttp_f[sk]):
        print("  [ERROR] IOC and TTP sample counts are not aligned.")
        return
    ...
    cfg = Config(
        device=str(device),
        old_loss_weight=args.old_loss_weight,
        replay_ratio=args.replay_mid_ratio,
        replay_warmup_ratio=args.replay_warmup_ratio,
        replay_mid_ratio=args.replay_mid_ratio,
        replay_late_ratio=args.replay_late_ratio,
    ...
    print(f"\n  [3/5] {cfg.n_folds}-fold cross validation...")
    fold_results = []
    timing = {k: [] for k in ["1.Base", "2.Replay", "3.Full"]}

    for fold, (tmp_tr, te_idx) in enumerate(skf.split(range(len(old_lbl)), old_lbl)):
        print(f'\n  {"="*60}')
        print(f"  Fold {fold+1}/{cfg.n_folds}")
        print(f'  {"="*60}')

        tr_idx, va_idx = train_test_split(
            tmp_tr, test_size=cfg.val_size, stratify=old_lbl[tmp_tr], random_state=42
        )
        ...
        ntr, nva = train_test_split(
            nt, test_size=cfg.val_size, stratify=new_lbl[nt], random_state=42
        )
        ng_tr = new_vidx[ntr]
        ng_va = new_vidx[nva]
        ng_te = new_vidx[nti]
        ...
        t0 = time.time()
        b_ioc = train_ioc(
            build_ioc(new_ioc, nc, cfg).to(device),
            old_ioc,
            g_tr,
            g_va,
            old_cw,
        ...
        b_ttp = train_ttp(
            build_ttp(old_ttp_f, nc, cfg).to(device),
            old_ttp_f,
            tr_idx,
            va_idx,
            old_lbl,
        ...
        print(f'  {"-"*W}')
        print(
            f'  {"Method":<26} | {"Old_Fusion":>8} | {"New_Fusion":>8} | '
            f'{"All_Fusion":>8} | {"IOC_Acc":>8} | {"TTP_Acc":>8} | '
            f'{"Fusion_BAcc":>8} | {"Time(s)":>7}'
        )
        print(f'  {"-"*W}')

        for name, (im, tm) in fold_models.items():
            r_old = get_metrics(
                im, tm, old_ioc, old_ttp_f, g_te, te_idx, old_lbl, cfg, device
            )
            r_new = get_metrics(
                im, tm, new_ioc, new_ttp_f, g_te_pure, lt_pure, new_lbl, cfg, device
            )
            r_all = get_metrics(
                im, tm, new_ioc, new_ttp_f, ng_te, nti_arr, new_lbl, cfg, device
            )
            tk = {
                "1. Base (Old Only)": "1.Base",
                "2. Incremental (Replay)": "2.Replay",
                "3. Full Finetune": "3.Full",
            }.get(name, "")
            t_s = timing[tk][-1] if tk else 0.0
            fold_res[name] = {"old": r_old, "new": r_new, "all": r_all, "time": t_s}
            print(
                f"  {name:<26} | "
                f'{r_old["or_acc"]:>8.4f} | '
                f'{r_new["or_acc"]:>8.4f} | '
    ...
    for mn in model_names:
        ...
        for sp in splits:
            summary[mn][sp] = {}
            for mt in metrics_k:
                vals = [f[mn][sp].get(mt, 0.0) for f in fold_results]
    ...
    W = 108
    print(f'\n  {"="*W}')
    print("   Final report: fusion Acc (5-fold mean/std)  ".center(W))
    print(f'  {"="*W}')
    print(
        f'  {"Method":<26} | {"Test_Old Fusion":^18} | {"Test_New Fusion":^18} | '
        f'{"Test_All Fusion":^18} | {"B-Acc(All)":^12} | {"Time(s)":^10}'
    )
    print(f'  {"-"*W}')
    ...
    print(f"\n   Replay vs Full Finetune")
    print(f'  {"-"*60}')
    desc = {
        "FM": ("Forgetting", "lower is better"),
        "BT": ("Backward", "higher is better"),
        "FT": ("Forward", "zero-shot transfer"),
        "Intransigence": ("Intransigence", "lower is better"),
        "PSR": ("Plasticity", "closer to 1 is better"),
    }
    for k, v in cl.items():
        zh, tip = desc[k]
        flag = ""
        if k == "FM":
            flag = "OK" if v <= 0 else ("WARN" if v < 0.05 else "BAD")
        if k == "BT":
            flag = "OK" if v >= 0 else "WARN"
        if k == "Intransigence":
            flag = "OK" if v < 0.05 else "WARN"
        if k == "PSR":
            flag = "OK" if 0.85 <= v <= 1.15 else "WARN"
    ...
    gap = rv - fv
    tsave = (ft - rt) / ft if ft > 0 else 0.0

    print("\n   Model size")
    print(f'  {"-"*60}')
    g1 = " " if abs(gap) < 0.01 else f"  {abs(gap):.2%}"
    g2 = " " if tsave > 0 else " "
    print("  1. Replay/Test_All fusion gap < 1%")
    print(f"    Full={fv:.4f}  Replay={rv:.4f}  gap={gap:+.4f}   {g1}")
    print("  2. Replay training is faster than full finetune")
    print(f"    Full={ft:.0f}s  Replay={rt:.0f}s  saved={tsave:.1%}   {g2}")
    ...
    print(f"    IOC (RGCN):       {n_ioc:>10,} ")
    print(f"    TTP (Transformer):{n_ttp:>10,} ")
    print(f"    Total:           {n_ioc+n_ttp:>10,}")

    results = {
        "config": {
            "replay_ratio": cfg.replay_ratio,
            "replay_warmup_ratio": cfg.replay_warmup_ratio,
            "replay_mid_ratio": cfg.replay_mid_ratio,
            "replay_late_ratio": cfg.replay_late_ratio,
